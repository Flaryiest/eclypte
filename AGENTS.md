...
- Dashboard routes are product code, not placeholders:
  - `/dashboard/new-edit`: saved-asset selection, edit pipeline creation, run streaming/poll fallback, preview/download, cancel/delete/redo.
  - `/dashboard/assets`: direct R2 uploads, asset library, manual analysis, YouTube song import, preview/download.
  - `/dashboard/synthesis`: reference queueing, consolidation, active prompt versions.
  - `/dashboard/renders`: render-output library.
  - `/dashboard/settings`: API/user/prompt health details.
- `web/src/services/eclypteApi.ts` is the typed browser client for the FastAPI v1 surface. Prefer extending it over ad hoc `fetch` calls in pages.
