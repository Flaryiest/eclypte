...
- `api/autopilot.py`: review-gated autopilot tick state machine — advances a per-user content queue (song import → cinematic 9:16 Reels edit with an energy-ranked ~15–22s trim window → auto-created publishing package awaiting approval), with combo dedupe, a daily target, and a 3-failure halt. Runs via the env-gated FastAPI lifespan loop (`ECLYPTE_AUTOPILOT=1`) or `POST /v1/autopilot/tick`.
