...
- Buffer publishing is review-gated. `/dashboard/publish` lets users edit captions/hashtags/notes, then queue or schedule an approved render through Buffer as an Instagram Reel. Public R2 copies under `public/publishing/{user_id}/{post_id}/` are created only when sending to Buffer; the page auto-checks Buffer once per selected post to back-fill the live permalink, and a manual "Refresh from Buffer" button re-checks on demand (surfacing Buffer errors).
...
- Dashboard routes are product code:
  ...
  - `/dashboard/publish`: review-gated Instagram Reels publishing queue for Buffer, including setup diagnostics, caption editing/regeneration, queue/schedule actions, posted/error metadata, an automatic one-shot Buffer status check for the selected post, and a manual "Refresh from Buffer" button to re-check on demand.
