...
- `api/prototyping/music/`: audio ingestion, Modal-backed allin1 music analysis, lyrics lookup, and optional R2 publishing. `search_synced_lyrics` (synced LRC only) is fetched opportunistically during analysis and stored as a `lyrics` asset for lyric overlays.
...
- `api/prototyping/edit/`: deterministic and agentic timeline planning, CLIP index build/query, reference ingestion/consolidation, timeline schemas/validation, a registry of creative overlay skills (`skills/` — text/masks composited over the reel, incl. data-driven synced-lyric overlays), MoviePy rendering (MP4 + poster frame), and R2-aware Modal render/index wrappers.
...
- Creative overlays (text/masks) come from a skill registry (`edit/skills/`) composited over the reel by the renderer. The agent places text/mask overlays via `finish_edit`; synced-lyric reels are data-driven — a per-edit `burn_lyrics` flag (new-edit toggle / autopilot state) expands the song's stored LRC into windowed `text.lyric` overlays at plan time. Adding/removing a skill is a single self-contained module; the agent tool enum, validators, and renderer all read the registry.
