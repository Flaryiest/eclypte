...
- `api/publishing.py`: review-gated Buffer publishing for Instagram Reels — Gen-Z-voiced OpenAI/fallback caption generation in the niche register — a short hook line, an `anime: X · song: Y` credit line, optionally ONE genuine fandom question or send-nudge, at most `MAX_HASHTAGS`=5 fandom-specific hashtags, and a repetition guard: the generator receives the account's `RECENT_CAPTION_WINDOW`=8 latest captions (`recent_account_captions`), retries once when the model echoes a recent hook (`caption_repeats_recent` — normalized first-line match), then falls back to the deterministic caption, which self-dedupes its hook; the copyable vibe examples were removed from the prompt after leaking verbatim into real captions (`_finalize_hashtags` bans generic discovery tags: `#fyp`, `#viral`, bare `#edit`/`#anime`, ...). The model is fed the source movie/anime + song names — resolved from the render's run lineage by `resolve_edit_source_names`, scrubbed by `_clean_media_name` (scene-release junk like `1080p`/`x265`/`[group]` is cut at the first junk token), and persisted on the post as `source_name`/`song_name`, public R2 media copies, Buffer GraphQL payloads (declares the Instagram `reel` post type; the send modes are `queue`=`addToQueue`, `schedule`/`now`=`customScheduled` — `now` posts immediately via a server-computed near-future `dueAt` from `immediate_due_at`, since Buffer has no instant-publish mode and rejects past `dueAt`), channel diagnostics, post-status refresh (`apply_buffer_status`) that marks a post `published` as soon as Buffer reports it sent (`sentAt`/sent status) and independently back-fills the permalink from `externalLink` when it appears, `send_post_to_buffer` (the shared send path used by both the `send-buffer` route and autopilot's auto-publish pass, raising typed `BufferConfigError`/`SendToBufferError`), and `BufferClient.delete_post` (the `deletePost` GraphQL mutation, treating `success: false` as a failure; mutation shape pending live-schema confirmation) backing the `cancel` route's veto of a queued/scheduled post. Direct Graph publishing (Phase B): `api/instagram_graph.py` is the stdlib-only Graph API client (pure `build_reel_container_params`/`parse_insights` + `GraphPublisher` — container create/poll/publish, `copyright_check_status`, insights, `ig_audio` search); `send_post_via_graph` mirrors `send_post_to_buffer` (stale-check → public MP4+poster copies → container with cover → canary veto on `matches_found` (post stays `ready`, `copyright_status` stamped, HTTP 409) → immediate publish, stamping `provider="graph"`/`ig_media_id`/`posted_at`); `resolve_publish_provider_env` picks the path and the send route/autopilot closure dispatch on it. Performance feedback loop (Phase 1): `BufferClient.get_post_metrics` pulls a sent post's per-post performance (GraphQL `post.metrics`/`metricsUpdatedAt`, tolerant parse, returns only metrics Buffer actually reported — absent is never zero); `apply_post_metrics` is the pure fold (stamps `metrics_checked_at` always, appends a `PostMetricsSnapshot` to `metrics_history` only when the reading changed — capped at the first 2 + last 10 entries — and never touches `last_error`/status); `performance_score` is the pure scorer (`log(primary metric)` minus `log(median primary metric)` across the post's ≤10 most recent published peers (primary = the post's `views` metric, falling back to `impressions`; the value must be > 0 to score), `None` under 5 scored posts — baseline-relative because reel counts are heavy-tailed).
...
- `.agent/`: agent/process assets, not runtime app code (`.superpowers/` is local-only, gitignored). `docs/`: process plans/specs — `docs/plans/task.md` (the superpowers task tracker), `docs/superpowers/plans/`, `docs/superpowers/specs/` — not runtime app code.
...
- `ECLYPTE_PUBLISH_PROVIDER` (`buffer` default | `graph`): which path sends approved posts. `graph` publishes straight through the Instagram Graph API (`api/instagram_graph.py`) — immediate posting with autopilot slot spacing, `cover_url` from the render poster, a `copyright_check_status` veto, and first-party insights metrics. Needs `ECLYPTE_IG_USER_ID` + `ECLYPTE_IG_ACCESS_TOKEN` (long-lived, Facebook-Login-connected professional account; `ECLYPTE_GRAPH_API_BASE` optional). Setup runbook in `api/COMMANDS.md`; `/healthz` reports `graph_publishing_configured`.
...
- Publishing: `GET /v1/publishing/config`, `GET /v1/publishing/posts` (each item carries a `performance_score`, computed against the account's recent published cohort), `POST /v1/publishing/posts`, `PATCH /v1/publishing/posts/{post_id}`, `POST /v1/publishing/posts/{post_id}/regenerate-caption`, `POST /v1/publishing/posts/{post_id}/send-buffer` (`mode`: `queue`/`schedule`/`now`), `POST /v1/publishing/posts/{post_id}/refresh-status` (also pulls the post's latest metrics when it's `published` — a pull failure logs and is swallowed, never surfaced as `last_error`), `POST /v1/publishing/posts/{post_id}/mark-posted`, `POST /v1/publishing/posts/{post_id}/cancel` (on a `queued`/`scheduled` post, deletes it from Buffer first — the human veto — failing with a 502 and an untouched post if Buffer refuses the delete).
- Autopilot: `GET /v1/autopilot`, `PATCH /v1/autopilot` (enable/disable, daily target, clear halt, and two default-off autonomy flags — `auto_pair`, `auto_publish`), `POST /v1/autopilot/queue`, `DELETE /v1/autopilot/queue/{item_id}`, `POST /v1/autopilot/tick` (manual tick). `api/autopilot.py` owns the review-gated tick state machine: queue items pair a saved film with a saved song (analysis is kicked off first when missing), and the tick starts fill-frame 9:16 Reels edit jobs (`reels_9_16`, `edit_focus="moment"` so the agent edits one scene rather than a whole-film montage) with an energy-ranked ~20–30s trim window (≈25s target) from the song's music analysis (section-anchored windows begin `CHORUS_LEAD_IN_SEC`=1.5s early — just enough for the hook shot to register before the drop), dedupes (video, song, window) combos twice over (exact 5s-bucket `used_combos` plus `used_windows` overlap rejection — a candidate overlapping a used window of the pair by >40% is a near-duplicate and is skipped; recycling clears both), halts after 3 consecutive failures, and auto-creates `ready` publishing packages (`auto_created=true`) that wait for human approval in the dashboard's Home feed. With `auto_pair` on, a replenish step (after advancing in-flight items, before starting new work) synthesizes one queue item per tick — consumed the same tick — whenever there's capacity and nothing pending: `select_next_pair` LRU-rotates saved films × songs (least-recently-paired first, via `AutopilotState.last_paired_at`), skipping pairs recorded in `exhausted_pairs` (every trim window already used); when every pair is exhausted it recycles the least-recently-paired one (clearing its `used_combos` window entries and its `exhausted_pairs` entry) instead of stalling, and stamps `recycling`/`waiting_for_library` on the state each evaluation — an empty library never halts, it just waits. With `auto_publish` on, `_auto_send_ready_posts` runs after `STATE_LOCK` releases (Buffer I/O is too slow to hold the lock across) and sends `ready`+`auto_created` posts to Buffer's queue via the shared `send_post_to_buffer`, skipping while autopilot is paused (`enabled=false`) or halted, backing off 30 minutes on a post's `last_error`, budgeting every send (per-pass cap of `daily_target` sends, stopping once queued posts reach 2×`daily_target` — re-checked per send via each send's returned record, so a first pass over an accumulated ready backlog trickles instead of dumping), and serialized by a non-blocking `SEND_LOCK` so a concurrent tick can't double-send; send failures record `last_error` on the post and never count toward the 3-failure halt. Under `ECLYPTE_PUBLISH_PROVIDER=graph` the auto-send pass IS the scheduler (Graph has no queue): `graph_slot_due` spaces publishes `86400/daily_target` seconds apart, one publish per open slot per pass, attempts capped at `daily_target`; a copyright veto stamps `last_error` for the 30-min backoff. The metrics pass dispatches per post: graph-published posts (`provider="graph"` + `ig_media_id`) pull first-party Graph insights (`fetch_graph_metrics` → `GRAPH_INSIGHT_METRICS`), Buffer posts keep `get_post_metrics`; `refresh-status` likewise backfills a graph post's permalink via `get_media` and pulls insights. Between the state pass and the send pass, a `_reconcile_buffer_statuses` pass (non-blocking `RECONCILE_LOCK`, hourly `status_checked_at` cadence, capped 10/pass) converges queued/scheduled posts with Buffer's real sent state server-side — so the send budget and the creation brake (replenish pauses, stamping `backlog_paused`, while unposted auto-created posts cover 2x `daily_target`) act on real counts even with the dashboard closed. After that send pass, a `_refresh_post_metrics` pass (also outside `STATE_LOCK`, serialized by its own non-blocking `METRICS_LOCK`) pulls Buffer's per-post metrics for every `published` post with a stored `buffer_post_id` — this pass runs regardless of `enabled`/`auto_pair`/`auto_publish`/halt, since review-gated posts want metrics too — on a 12h cadence (`METRICS_REFRESH_INTERVAL_SEC`), capped at `METRICS_MAX_PER_PASS`=20 posts/pass, retiring posts older than `METRICS_RETENTION_DAYS`=30 days, and saving onto a freshly reloaded record via `apply_post_metrics` (skipping a post that vanished or left `published` mid-fetch, so a concurrent permalink backfill/mark-posted/cancel is never reverted); a fetch failure logs and stamps `metrics_checked_at` only — never `last_error`, never the halt. One real consequence: pausing autopilot (`enabled=false`) pauses this background pass too, since the whole tick is skipped while paused — the manual `refresh-status` route still pulls metrics on demand. State lives in R2 at `users/{user_id}/autopilot/state.json` with enabled-user markers under `autopilot/enabled/`.
...
Default asset lists hide archived assets, render outputs, render posters, and source posters. Use `kind=render_output` (or `kind=source_poster`) to list them explicitly. Archive/restore instead of hard deletion for normal dashboard lifecycle. `AssetSummary.poster` carries an optional `FileVersionInput` ref to the asset's thumbnail — resolved from the completed `video_analysis` run's `source_poster_file_id`/`source_poster_version_id` outputs for a `source_video` asset, or from the `render_poster_file_id`/`render_poster_version_id` outputs for a `render_output` asset — and `AssetSummary.poster_url` carries a ready-to-use signed URL for it, presigned locally in the listing from the deterministic version blob key (zero extra network ops; `DOWNLOAD_URL_EXPIRES_IN` is 3600s). Publishing post responses are `PublishingPostView` (the stored record plus per-response `poster_url`/`render_url`; signed URLs are never persisted — posts store only `render_poster_file_id/version_id` refs, captured at creation and lazily backfilled for legacy posts on first list). `PublishingPostRecord` also carries the performance-feedback fields (Phase 1): `metrics` (latest normalized name→value map pulled from Buffer — a name absent from the map was never reported, not zero), `metrics_updated_at` (Buffer's own ingestion stamp), `metrics_checked_at` (our poll cadence control), `metrics_history` (a list of `PostMetricsSnapshot{captured_at, metrics}`, capped at the first 2 + last 10 readings), and `source_video_file_id`/`song_file_id` (lineage ids captured at packaging from the render run's inputs — Phase 2 attribution enablers, unused by Phase 1). Phase B adds `provider` (stamped `"graph"` at graph-send time; default `"buffer"`), `ig_container_id`/`ig_media_id`, and `copyright_status` (`clean`/`matches_found` canary result) — all additive. `PublishingPostView` additionally carries `performance_score` — computed per `GET /v1/publishing/posts` list response only (null on single-post responses), never persisted.
...
- `reels_cinematic`: 1080x1920, letterbox — the full widescreen picture centered on a native vertical canvas with black bars baked in (manual composes only; autopilot switched to `reels_9_16` — letterboxed reels sit on Instagram's official demotion list).
...
- `synthesis/timeline_schema.py`: Pydantic timeline schema. `Timeline.overlays` carries optional `Overlay` items (`skill_id` + window + validated `params`), composited over the shots. `AudioSpec.fade_out_sec`/`OutputSpec.fade_out_sec` carry the end-of-reel fade lengths; the shared `tail_fades_for(duration_sec)` helper returns `(audio_fade, video_fade)` — reels ≤`SHORT_REEL_MAX_SEC`=40s get a 0.3s audio-only fade and NO fade-to-black (a visible fade kills the loop/replay signal), long form keeps the classic 2.5s fade on both tracks clamped to ≤ ⅓ of the piece.
...
- Tools are `query_clips(query, top_k)` and `finish_edit(timeline)`; timeline items optionally carry `transition_in` (`cut`/`flash`/`crossfade`) and `effect` (`punch_in`/`speed_ramp` — `freeze` was retired from the plannable set in Aug 2026: a held frame reads as a broken video; the adapter drops it if a stale prompt still emits it, and the renderer keeps legacy support for stored timelines). `finish_edit` also takes optional top-level `overlays`, `grade`, and `lyrics` ({enabled, font, style, section_styles?, accent_color?} — the kinetic-lyrics plan; the overlays skill_id enum excludes `grade`/`lyrics` kinds so lyrics can't be double-placed).
...
- Source-coverage guidance is per-run and switched by `edit_focus` (`EditJobRequest`/`TimelineRequest` field, default `full_source`; recorded in run inputs and honored by redo): `full_source` injects the span-the-full-source block (short/trimmed edits cover the whole film); `moment` — autopilot's default — injects a FOCUSED MOMENT EDIT block that explicitly overrides the span rule (so stale stored prompt versions can't fight it) and briefs the agent to build the reel around ONE scene/sequence/character arc, hook-first. Injected into per-run user content so it applies regardless of the active prompt version; nothing enforces either (no validator/forced spread).
...
- Both render paths apply the end-of-reel fades: native ffmpeg emits `afade`/`fade`; the MoviePy fallback uses `render/fades.py` (`audio_fade_out`/`video_fade_out` via `clip.transform`); both skip a zero-length fade. Lengths come from `timeline.audio.fade_out_sec`/`timeline.output.fade_out_sec`, set by the adapter through `tail_fades_for`: short reels (≤40s) get a 0.3s audio-only fade and a HARD video ending (a fade-to-black reads as an ending mid-scroll and costs replays), long form keeps the 2.5s audio+video fade (≤ ⅓ of the piece). The fade is just filters/transforms, so it does not push a cuts/crossfade timeline off the fast native path, and the mid-clip poster is unaffected.
...
## Current Focus & Next Steps (August 2026)

The newest push is **reach hygiene** (Phase A of the reels-reach-recovery plan — spec/plans in `docs/superpowers/`): the Aug 2026 deep-research pass found the account's minimal views trace to distribution-level penalties, so autopilot reels switched to fill-frame `reels_9_16` (letterbox is officially demoted), single-scene `edit_focus="moment"` plans, a hard ending (0.3s audio-only tail fade via `tail_fades_for`, no fade-to-black), a 1.5s chorus lead-in, niche-register captions capped at 5 fandom hashtags (no `#fyp`), scene-release junk stripped from media names, `used_windows` overlap dedupe (no near-duplicate reels), and `first_shot` hook telemetry in the sync report. **Redeploy `eclypte-render-r2`** (bundled `timeline_schema.py` changed — behavior rides existing fields, so old images render new timelines correctly, but keep image and repo in lockstep). Phase B (direct Graph API publishing) is implemented behind `ECLYPTE_PUBLISH_PROVIDER=graph` (default `buffer`): `cover_url` covers, the `copyright_check_status` canary, slot-spaced auto-publish, and first-party insights metrics. Still pending the operator's Meta app/token setup + the Task 0 live probes in `docs/superpowers/plans/2026-08-04-graph-publishing.md` — and the probe-gated attached-licensed-audio step (Task 3b) is NOT built until those probes settle the audio-offset/sync question; burned-in audio remains until then.

The prior push is edit quality — making reels *feel* musically cut, not just land on beats. Phase 1 (the rhythm engine, `synthesis/rhythm.py`) is implemented: downbeat-preferred snapping with an early-cut lead, impact→downbeat source-window registration, tempo-scaled per-section pacing (agent guidance + a deterministic split backstop for fast sections), motion/impact-enriched `query_clips` results, and `timeline_sync_report` telemetry per plan. All of it stays on the fast native ffmpeg render path and needs no Modal redeploys.

Autopilot context (previous push, still live — updated by the reach-hygiene push): review-gated packages rendered as `reels_9_16` fill-frame (was `reels_cinematic` letterbox) from ~20–30s energy-ranked windows (≈25s, 1.5s chorus lead-in, was 5s), a hard-ending tail (0.3s audio-only fade, no fade-to-black — was a 2.5s audio+video fade), single-scene `edit_focus="moment"` plans, and a CRF 18 encode. Captions name the source movie/anime + song with context-relevant AI hashtags. Autonomous pairing + posting (this push, both off by default) build on top: `auto_pair` replenishes the queue itself via LRU film×song rotation with exhaustion/recycle so it never runs dry; `auto_publish` drains `ready` auto-created packages into Buffer's queue on a tick. Buffer's own channel posting schedule (configured in Buffer's dashboard) supplies the actual posting cadence and doubles as the human veto window — a queued/scheduled post can still be pulled back via the extended `cancel` route, which now deletes it from Buffer first.

Performance feedback loop, Phase 1 (this push): the autopilot tick now closes the loop from "posted" back to "how did it do" — a metrics-refresh pass ingests Buffer's per-post metrics (views/impressions/likes/comments/…, whichever Buffer actually reports) into `PublishingPostRecord.metrics`/`metrics_history`, and `performance_score` scores each published reel log-relative to the account's own trailing median (see `api/publishing.py` and the Storage Substrate / Routes sections above for the exact mechanics). The dashboard's Home feed surfaces a compact metrics line + relative chip on posted cards and a dated history in the review sheet. Buffer post metrics are licensed for personal workflows/automations only — fine for the current single-operator product, a known blocker before any multi-tenant launch (upgrade path: direct Meta "Instagram API with Instagram Login", not built). Phase 2 (adaptive pairing/window steering informed by this data) is a future push; this phase is ingestion + visibility only.

Operational checklist:

- QA the first reach-hygiene reel: full-frame (no black bars), one coherent scene (not a whole-film montage), a hard video ending on a strong closing beat (the edit must NOT re-show its opening shot at the end), ≤5 fandom hashtags with the `anime: X · song: Y` credit line, and `timeline_sync_report.first_shot.impact_backed=true`. Run the Phase 0 operator diagnosis (Account Status, follower/non-follower split, logged-out visibility, Creator account type) from the reach-recovery spec.
...
- Autonomy go-live (one-time): set the Buffer channel's posting schedule to the target cadence (e.g. 2 slots/day) in Buffer's dashboard and match `daily_target` to it, flip "Pick pairs for me" + "Post without review" on the Home feed, then run the deploy-time veto check — queue one post and cancel it, confirming the `deletePost` mutation round-trips against Buffer's live schema (its shape was written against documented conventions, never introspected). Do this before leaving `auto_publish` unattended.

All four phases of the edit-quality roadmap are implemented: the rhythm engine (Phase 1), the ffmpeg polish foundation (Phase 2 — skill kinds, capability-driven dispatch, flash/punch_in/vignette/drawtext ported native; freeze was also ported, then retired from planning in Aug 2026), the polish catalog (Phase 3 — grade presets, impact.shake + auto-accents, real speed_ramp), and reference-derived style profiles (Phase 4 — plan-time rhythm overrides from reference metrics). On top of them sits **kinetic lyrics** (July 2026): the `lyrics.kinetic` skill, ASS/libass rendering, the footage-adaptive layout engine, the pinned font catalog, and agent default-on lyric plans. **Redeploy `eclypte-render-r2` before the next real render** — the bundled render/skills/schema code changed AND the image itself changed (kinetic fonts + libass assert).
...
- Autonomy-loop polish (accepted final-review follow-ups): prune `exhausted_pairs` for deleted/archived assets (only `last_paired_at` is pruned today); background the manual tick route's send pass (it currently does synchronous Buffer I/O in the request); unify the "· by autopilot" vs "· auto-paired" copy voice.
- Performance feedback loop, Phase 2: adaptive pairing/window steering from `performance_score`/`metrics` (no experiments or virality prediction yet); revisit the Buffer-metrics personal-workflows licensing constraint (route B: direct Meta) before any multi-tenant launch.
