...
- Autonomy-loop polish (accepted final-review follow-ups): re-check the queued backstop between sends within one pass (currently computed once per pass, so a ready backlog can overshoot 2×`daily_target` slightly); prune `exhausted_pairs` for deleted/archived assets (only `last_paired_at` is pruned today); background the manual tick route's send pass (it currently does synchronous Buffer I/O in the request); unify the "· by autopilot" vs "· auto-paired" copy voice.
