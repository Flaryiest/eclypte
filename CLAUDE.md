...
- Autopilot: `GET /v1/autopilot`, `PATCH /v1/autopilot` (enable/disable, daily target, clear halt), `POST /v1/autopilot/queue`, `DELETE /v1/autopilot/queue/{item_id}`, `POST /v1/autopilot/tick` (manual tick). `api/autopilot.py` owns the review-gated tick state machine: it imports YouTube songs, starts cinematic 9:16 Reels edit jobs (`reels_cinematic`) with an energy-ranked ~15–22s trim window from the song's music analysis (section-anchored windows begin `CHORUS_LEAD_IN_SEC`≈2.5s early so a chorus reel captures the build-in), dedupes (video, song, window) combos, halts after 3 consecutive failures, and auto-creates `ready` publishing packages (`auto_created=true`) that wait for human approval on the publish page. State lives in R2 at `users/{user_id}/autopilot/state.json` with enabled-user markers under `autopilot/enabled/`.
...
- `skills/`: registry of creative overlay skills (text/masks) composited over the reel. Each skill is one self-contained module (`text.hook`, `text.caption`, `text.lower_third`, `text.lyric`, `mask.vignette`) declaring `id`, an agent-facing `description`, a Pydantic `params_model`, and `build_layers(overlay, ctx)`. `text.lyric` is data-driven (auto-expanded from synced lyrics), not agent-placed.
- `synthesis/lyrics.py`: pure LRC parsing (`parse_lrc`) + windowed expansion (`expand_lyrics_overlays`) — offsets/clips synced lyric lines into the trimmed audio window and emits `text.lyric` overlay dicts for `adapt(overlays=...)`. No moviepy. Gated by a per-edit `burn_lyrics` flag (`EditJobRequest.burn_lyrics` → `run_edit_pipeline` → `_run_agent_timeline_plan._build_lyric_overlays`, which finds the stored `lyrics` asset or fetches on demand by song name). The new-edit UI and an autopilot `burn_lyrics` state toggle set it; lyric overlays merge with the agent's in the agent planning path only. Skills self-register on import (`register(...)`); the agent tool enum, validators, and renderer all read the registry (`skills.ids()` / `skills.get()` / `skills.agent_catalog()`), so adding/removing a skill is a single module plus one import line in `skills/__init__.py`. Skill metadata stays moviepy-free (Railway control plane); `build_layers` imports moviepy/numpy lazily and runs only inside the Modal renderer.
...
- The agent is instructed to never select black frames, solid colors, title cards, logos, or end credits, and to span the end of the *content* (not the trailing credits/black). The hard guarantee is data-side: `query_clips` results are content-filtered in the CLIP index (see `index/storage_modal.py`), so dead frames are not returned regardless of the prompt.
