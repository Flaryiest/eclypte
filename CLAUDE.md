...
- QA the first reach-hygiene reel: full-frame (no black bars), one coherent scene (not a whole-film montage), a hard video ending that cuts back cleanly toward the opening, ≤5 fandom hashtags with the `anime: X · song: Y` credit line, and `timeline_sync_report.first_shot.impact_backed=true`. Run the Phase 0 operator diagnosis (Account Status, follower/non-follower split, logged-out visibility, Creator account type) from the reach-recovery spec.
