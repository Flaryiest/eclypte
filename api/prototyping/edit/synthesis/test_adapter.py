...
def test_adapt_maps_agent_transitions_and_effects():
    ...
    assert tl.shots[1].effects == []
    # unknown values fall back to plain cut / no effects
    assert tl.shots[2].transition_in.type == "cut"
    assert tl.shots[2].effects == []


def test_adapt_ignores_retired_freeze_effect():
    # freeze was removed from the plannable set — a held frame reads as a
    # broken video. An agent (or a stale stored prompt) that still emits it
    # gets plain playback on every shot, opener and interior alike.
    agent = [
    ...
    assert all(shot.effects == [] for shot in tl.shots)
