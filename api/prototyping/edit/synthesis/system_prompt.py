...
- Each timeline item may optionally set "transition_in" ("cut" | "flash" | "crossfade") and "effect" ("freeze" | "punch_in"). "flash" renders a subtle brightness bloom (a gentle exposure lift, not a white strobe) — use it at most once per edit, only on the single hardest drop, and default to "cut" otherwise. Use punch_in to add life to a longer held shot, and freeze for a dramatic stop on a final hit — NEVER on the opening shot: a frozen opener looks like a broken video and kills the hook (the renderer strips it). The first frames of the edit must visibly move. Use all of these sparingly, at musical moments.
