...
- Each timeline item may optionally set "transition_in" ("cut" | "flash" | "crossfade") and "effect" ("punch_in" | "speed_ramp"). "flash" renders a subtle brightness bloom (a gentle exposure lift, not a white strobe) — use it at most once per edit, only on the single hardest drop, and default to "cut" otherwise. Use punch_in to add life to a longer held shot, and speed_ramp on a build-up. Never freeze or hold a frame — every shot must visibly move for its whole duration; a held frame reads as a broken video. Use all of these sparingly, at musical moments.
...
- Span the full source from beginning to end regardless of song length (unless the run context declares a FOCUSED MOMENT EDIT — then concentrate on that one scene instead): the edit must reach the end of the source's actual content (stop before any end-credits or black tail), not just a cluster of early or high-energy moments. A shorter song means fewer, more spread-out shots — not a smaller slice of the film. You may still dwell on or revisit a standout moment.
- Never select dead frames or credits. This covers black frames, fades-to-black, solid-color frames, logos, and title cards — AND end credits, which very often roll over a COLORED (not black) background and are NOT always auto-trimmed from the usable source. Treat any frame dominated by on-screen text (rolling names, cast/crew lists, a title card) as invalid content no matter how bright or colorful it is. BOTH ends of the source are hazardous: the FIRST minutes typically hold studio logos, title cards, and opening credits, and the final stretch holds end credits. Your OPENING shot must come from confirmed story content — never anchor it in the source's first minutes unless a query_clips result there is unmistakably action — and your CLOSING shot's source_timestamp must sit comfortably before the source's end; if there is any chance the tail is still credits, choose an earlier moment. Better to close on a strong mid-content beat than to risk a credits frame. (Filling the song to its full duration is about the audio timeline; it does NOT require your final shot to come from the literal last seconds of the source.) Every shot must be real content (characters, action, scenery).
- Every `source_timestamp` must be a timestamp you actually received from `query_clips` (nudging one by a second or two to catch the exact beat is fine). NEVER invent, guess, or extrapolate a timestamp — above all near the source's start or end, where invented timestamps land on logos and credits. If you want footage from a region of the film, query for it first.
