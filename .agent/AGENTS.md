...
- **Canonical Runbook**: Use `api/COMMANDS.md` for exact setup, Modal, planning, rendering, storage, and test commands. For R2-backed flows, local secrets should live in `api/.env` or environment variables; never commit secrets, exported cookies, or tokens.
...
- **Current Repo Map**:
  ...
  - `api/workflows.py`: Railway-style background workflow runner. It creates/upserts run manifests, reuses or schedules analyses, calls Modal wrappers for heavy media work, runs OpenAI/CLIP-backed timeline planning, and records progress.
  - `api/storage/`: shared Cloudflare R2-oriented storage substrate with `R2Config`, object-store factory helpers, canonical key builders, typed refs/models, an S3-compatible client, a repository layer, presigned URL helpers, upload reservations, and run-status helpers. The REST API uses it as the v1 metadata/artifact store, while some older prototype CLIs still read and write local files or Modal volumes directly.
  - `api/prototyping/music/`: the pure analysis helper, lyric lookup, and the Modal-backed music analysis app (`eclypte-analysis`). Audio ingestion (upload transcode via `api/audio_convert.py`) and R2 publishing live in the control plane (`api/workflows.py`).
  ...
  - `api/prototyping/edit/`: the main AMV pipeline. Reference ingest consolidates reference AMVs into `knowledge/references.md`, and timeline synthesis uses OpenAI Responses plus Modal CLIP retrieval to build timelines before rendering.
  - `api/prototyping/edit/render/`: timeline-to-MP4 renderer that dispatches between a native ffmpeg filtergraph path (cuts/crossfade) and a MoviePy fallback (overlays/effects); both paths also write a poster frame.
  ...
  - `api/prototyping/edit/synthesis/`: timeline schema, validator, agent loop, adapter, and tests.
  - `web/`: Next.js 16 + React 19 frontend with Clerk auth, built-out marketing pages (landing, three-tier pricing, and a "Screening Room" demo with poster-first lazy video), and a real dashboard console for uploads, saved assets, edit pipelines, Buffer publishing, synthesis prompt management, render browsing, and settings.
...
- **Cloud REST API V1 Routes**:
  ...
  - Workflows: `POST /v1/music/analyses`, `POST /v1/music/conversions`, `POST /v1/video/analyses`, `POST /v1/timelines`, `POST /v1/renders`, `POST /v1/edits`
  ...
  - Publishing: `GET /v1/publishing/config`, `GET|POST /v1/publishing/posts`, `PATCH /v1/publishing/posts/{post_id}`, and `{post_id}/regenerate-caption|send-buffer|refresh-status|mark-posted|cancel` — review-gated Buffer Instagram Reels publishing.
  ...
  - Internal: `POST /internal/progress` (requires `X-Eclypte-Internal-Token`).
...
- **Music Workflow Status**:
  ...
  - `api/prototyping/music/lyrics.py` redirects `syncedlyrics` cache writes (via `LOCALAPPDATA`) to `./content/.cache` relative to the working directory on Windows so local `AppData` permission issues do not break or warn during normal runs.
