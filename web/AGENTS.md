...
- The edit job also carries `burnLyrics` (sent as `burn_lyrics`). When set and the song has a stored synced-lyrics asset, agent planning overlays the windowed lyrics (`text.lyric`) on the reel. Lyrics are sourced server-side during analysis; the toggle only controls display, and applies to the agent path only.
